...
## Implementação de API de transferências
  +  Foi utilizado o pattern chain responsability para executar uma sequencia de validacões
e também para futuras implementações é fácil de alterar.
  + Para resolver o problema do Bacen eu utilizei a mensageria da aws (SNS) e também fiz uma implementacao.
Um ponto interessante é que possuímos duas maneiras de resolver, sendo com retentativas na fila ou retentativas
na chamada http. Foi implementado por http, mas ambos funcionam muito bem.
  + Para visibilidade poderíamos utilizar o CloudWatch. Onde poderíamos entender o logs de todas as aplicações.
  + Para deploy seria utilizado o EKS, onde poderíamos também visualizar sobre a saúde de nossas apis.
  + Melhoria na arquitetura já foi aplicada, onde seria chamada do maneira assincrona, por exemplo mensageria a api do bacen.
  + Documentação das rotas foi feita com Swagger.

<p>
  <img src="images/aws-diagram.png" alt="Exemplo de Imagem">
</p>